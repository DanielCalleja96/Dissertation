...
def get_box_from_json(file):    
    ...
    for box in boxes:
        name= boxes.get('label')
        name= unicodedata.normalize('NFKD', name).encode('ascii','ignore')
        xmin= int(boxes.get('topleft').get('x'))
        ymin= int(boxes.get('topleft').get('y')) 
        xmax= int(boxes.get('bottomright').get('x'))
        ymax= int(boxes.get('bottomright').get('y'))
...
def find_accuracy(path,jpath):
    ...
    for b,file in enumerate(glob.glob("*.xml")):
        ...
        d_path=jpath+file.split(".xml")[0]+".json"
        print(d_path)
...
def main():
    annotation_path="C:/dark/darkflow-master/LISA/LisaTestAnnotations/"
    json_path="C:/dark/darkflow-master/LISA/TinyYOLO/AccuracyJSON/"
